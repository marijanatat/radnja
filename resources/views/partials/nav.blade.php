            ...
            <div class="dropdown-content">
              @foreach ($category->children as $child)
                ...
                {{-- <div class="" style="margin-top:-10;margin-left: 200px;margin-right:-100px;background-color: transparent; z-index:10 ;" > --}}
                  @if (!$child->isLeaf())
                  <div class="uppercase"  >
                    <a data-turbolinks="false" href="{{route('shop.index', ['category' => $child->slug])}}" class="text-lg text-gray-700 hover:text-teal-300">{{$child->name}}</a>
                  </div>
                    @foreach ($child->children as $ch)
    
                    <div class="flex-col">
                      <a data-turbolinks="false" href="{{route('shop.index', ['category' => $ch->slug])}}" class="text-gray-700 text-sm hover:text-teal-300">{{$ch->name}}</a>
                    </div>
                    @endforeach
                    @else
                    <div class="flex-col">
                      <a data-turbolinks="false" href="{{route('shop.index', ['category' => $child->id])}}" class="text-gray-700 hover:text-teal-300">{{$child->name}}</a>
                    </div>
                    @endif
                  </div>
